...
class Model(object):
    ...
    # def compute_perplexity_old(self, pred_logits, TRUE_CLICKS, MASK):
    #     '''
    ...
    def compute_perplexity(self, pred_logits, TRUE_CLICKS, MASK):
        ...
        Compute the perplexity
        '''
        session_num = pred_logits.shape[1] // self.max_d_num
        pos_logits = torch.log2(pred_logits + MINF)
        neg_logits = torch.log2(1. - pred_logits + MINF)
        perplexity_at_rank = torch.where(TRUE_CLICKS == 1, pos_logits, neg_logits)
        perplexity_at_rank = torch.where(MASK == True, perplexity_at_rank, torch.zeros(perplexity_at_rank.shape, device=device))
        for session_idx in range(1, session_num):
            perplexity_at_rank[:, :self.max_d_num] += perplexity_at_rank[:, self.max_d_num * session_idx:self.max_d_num * session_idx + self.max_d_num]
        perplexity_at_rank = perplexity_at_rank[:, :self.max_d_num].sum(dim=0)
        return perplexity_at_rank
    ...
    def _train_epoch(self, train_batches, dataset, metric_save, patience, step_pbar):
        ...
        for b_idx, batch in enumerate(train_batches):
            ...
            MASK = (MASK == 1)
            query_num = MASK.sum() // self.max_d_num
            if use_cuda:
    ...
    def evaluate(self, eval_batches, dataset):
        ...
        perplexity_at_rank = torch.zeros(self.max_d_num, device=device, dtype=torch.float) # max_d_num docs per query
        # pdb.set_trace()
        with torch.no_grad():
    ...
    def predict(self, predict_batches, dataset):
        total_logits = []
        # pdb.set_trace()
        with torch.no_grad():
            for b_idx, batch in enumerate(predict_batches):
                self.model.eval()
                # true_relevances_batches = batch['relevances']
                pred_logits, pred_rels = self.model(batch['qids'], batch['uids'], batch['vids'], batch['clicks'])
                relevances_batches = torch.zeros(pred_logits.shape[0], self.max_d_num)
                ...
                total_logits.append(pred_logits)

        total_logits = torch.cat(tuple(total_logits))
        return total_logits
