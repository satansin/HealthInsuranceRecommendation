...
class DGATLayer(nn.Module):
    def __init__(self, args, query_size, doc_size, vtype_size, dataset):
        ...
        self.pos_embedding = nn.Embedding(self.max_d_num, self.args.pos_embed_size)
    ...
    def forward(self, qids, uids, vids, clicks, use_gnn=True):
        ...
        pos_embedding = self.pos_embedding.weight.unsqueeze(dim=0).repeat(batch_size, seq_len // self.max_d_num, 1)  # [batch_size, seq_len, embed_size]
    ...
    def interact_neighs(self, qids, uids):
        ...
        qids_extended = QIDS.unsqueeze(dim=2).repeat(1, 1, self.max_d_num).view(batch_size, seq_len) # [batch_size, seq_len]
...
class QueryEncoder(nn.Module):
    ...
    def forward(self, qid_embed):
        ...
        query_outputs = query_outputs.repeat(1, 1, self.max_d_num).view(batch_size, self.max_d_num * session_num, self.args.hidden_size) # [batch_size, seq_len, hidden_size]
