...
def parse_args():
    ...
    parser.add_argument('--predict', action='store_true',
                        help='perform real prediction on new (predict) set')
...
def valid(args, dataset):
    ...
    for i in range(args.num_iter):
        ...
        print('final_valid/click_loss:', valid_click_loss)
...
def rank(args, dataset):
    ...
    although the clicks are filled in the label_set, the results are not affected by the clicks value
    that is, the same label results are obtained if all the clicks are set to 0
    """
    logger = logging.getLogger("GraphCM")
    logger.info('Initialize the model...')
    model = Model(args, dataset.query_size, dataset.doc_size, dataset.vtype_size, dataset)
    ...
    for i in range(args.num_iter):
        ...
        for trunc_level in trunc_levels:
            ...
            print('For i =', i)
            print('final_NDCG/avg_{}'.format(trunc_level), sum_ndcgs[trunc_level] / (i + 1))
            print('final_NDCG/{}'.format(trunc_level), ndcgs[trunc_level])

def predict(args, dataset):
    """
    really predict new data
    """
    logger = logging.getLogger("GraphCM")
    logger.info('Initialize the model...')
    model = Model(args, dataset.query_size, dataset.doc_size, dataset.vtype_size, dataset)
    ...
    predict_batches = dataset.gen_mini_batches('predict', dataset.predictset_size, shuffle=False)
    total_logits = model.predict(predict_batches, dataset)

    # np.savetxt(os.path.join('data', args.dataset, 'predict_results.txt'), total_logits.cpu().numpy())
    np.savetxt(os.path.join(dataset.data_dir, 'predict_results.txt'), total_logits.cpu().numpy())
...
def run():
    ...
    if args.log_dir:
        ...
        ## change : to - since in win we cannot have : in filename
        file_handler = logging.FileHandler(args.log_dir + time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time())) + '.txt')
